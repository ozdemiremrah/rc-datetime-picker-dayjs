...
class Day extends Component {
  ...
  _renderDay = (week, day) => {
    ...
    const month = isNextMonth 
      ? _dayjs.clone().add(1, 'month') 
      : isPrevMonth 
        ? _dayjs.clone().subtract(1, 'month')
        : _dayjs.clone();
  ...
  render() {
    ...
    const _dayjs = this.state.dayjs;
    const firstDay = _dayjs.clone().date(1).day();
    const endOfThisMonth = _dayjs.clone().endOf('month').date();
    const endOfLastMonth = _dayjs.clone().subtract(1, 'month').endOf('month').date();
    const days = [].concat(
    ...
    return (
      <div className="calendar-days" style={style}>
        <div className="calendar-nav">
          ...
          <span className="current-date" onClick={changePanel.bind(this, 'month', _dayjs)}>{_dayjs.format(dayFormat)}</span>
